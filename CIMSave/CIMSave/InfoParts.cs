...
namespace CIMCollect
{
    ...
    public class InfoParts
    {
        ...
        [DataMember]
        public string Server { get; set; }      // computer name -> column "Server"
        [DataMember]
        ...
        public void ToJsonFile()
        {
            ...
            CIMSave.GZfileIO.WriteStringToGZ($"{Server}_{Set}.Json.gz", ToJson());
        }

        public static InfoParts FromJsonFile(string fileName)
        {
            var json = CIMSave.GZfileIO.ReadGZtoString(fileName);
            return FromJson(json);
        }

        public static InfoParts FromJson(string json)
        {
            InfoParts pInfo;
            try
            {
                using (var ms = new MemoryStream(Encoding.Unicode.GetBytes(json)))
                {
                    ...
                    DataContractJsonSerializer deserializer = new DataContractJsonSerializer(typeof(InfoParts));
                    pInfo = (InfoParts)deserializer.ReadObject(ms);
                    //pInfo.Good = true;
                }
            }
            catch (Exception ex)
